    ...
    def __init__(self, emb_dim, num_words, sentence_length, atten_sen_len,
        ...
        with tf.name_scope("attention"):
            ...
            for i in input_tensors:
                ...
                #sum = tf.math.exp(sum)
                #print("shape of exp(sum): ", sum)
                output.append(sum)
        
            self.h_atten = tf.concat(output, axis=1)
            print("shape of self.h_atten: ", self.h_atten)
            sum_over = tf.math.reduce_sum(self.h_atten, axis=1, keepdims=True)
            print("shape of sum_over: ", sum_over)
            imp_matrix = []
            for i in output:
                imp_matrix.append(tf.divide(i, sum_over))
            
            self.h_atten = tf.concat(imp_matrix, axis=1)
